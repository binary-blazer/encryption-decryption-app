...
import { faArrowRight, faWaveSquare, faFont } from "@fortawesome/free-solid-svg-icons";
import "./App.css";
...
function caesarCipher(text: string, shift: number) {
    return text.replace(/[a-z]/gi, (char) => {
        ...
        return String.fromCharCode(offset < 0 ? offset + 26 + start : offset + start);
    });
}
...
function App() {
    const [encryptionMethod, setEncryptionMethod] = useState(encryptionMethods[0].name);
    ...
    function handleEncrypt() {
        ...
        switch (encryptionMethod) {
            case "Gartenzaun":
                result = railFenceCipherEncrypt(inputText, 2);
                break;
            case "Cäsar":
    ...
    function handleDecrypt() {
        ...
        switch (encryptionMethod) {
            case "Gartenzaun":
                result = railFenceCipherDecrypt(inputText, 3);
                break;
            case "Cäsar":
                result = caesarCipher(inputText, -caesarShift);
                break;
        ...
        <main className="container">
            <div className="flex-row">
                <div className="column sidebar">
                    ...
                    <div className="tabs">
                        {encryptionMethods.map((method) => (
                            ...
                            >
                                <FontAwesomeIcon icon={method.icon} /> {method.name}
                            </button>
                        ))}
                    </div>
                </div>
                <div className="column content">
                    ...
                    <p>Information about {encryptionMethod} encryption method.</p>
